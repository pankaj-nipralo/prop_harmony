...
const BookkeepingHeader = ({
  ...
  const handleExport = async (format) => {
    ...
    try {
      ...
      if (format === "PDF") {
        filename = exportToPDF(transactions, filters);
        ...
        notification.innerHTML = `
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="text-sm opacity-90">Report downloaded: ${filename}</div>
            </div>
          </div>
        `;
        ...
        filename = exportToExcel(transactions, filters);
        // Create success notification
        const notification = document.createElement("div");
        notification.className =
          "fixed z-50 max-w-md px-6 py-4 text-white bg-green-500 rounded-lg shadow-lg top-4 right-4";
