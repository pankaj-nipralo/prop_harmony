    ...
    reviewsList: [
      {
        ...
        reviewText:
          "The property is well-maintained and the landlord is very responsive to any issues. The location is perfect and all amenities work perfectly. Highly recommend this property to anyone looking for a quality rental experience.",
        ...
        landlordResponse: {
          responseText:
            "Thank you for your wonderful review! We're delighted to hear about your positive experience. We strive to maintain high standards and your feedback motivates us to continue providing excellent service.",
      ...
      },
      {
        ...
        landlordResponse: {
          responseText:
            "Thank you for your positive feedback! We're working on improving parking availability and appreciate your patience. We're glad you're enjoying the modern amenities.",
      ...
      },
      {
        ...
        landlordResponse: {
          responseText:
            "Thank you so much for your kind words! It's tenants like you who make our job rewarding. We're committed to maintaining these high standards.",
...
// Tenant-specific category labels
export const tenantRatingCategories = [
  { key: "communication", label: "Payment Reliability", icon: "CreditCard" },
  { key: "maintenance", label: "Property Care", icon: "Home" },
  { key: "location", label: "Neighborhood Respect", icon: "Users" },
  { key: "value", label: "Lease Compliance", icon: "FileCheck" },
  { key: "cleanliness", label: "Property Condition", icon: "Sparkles" },
];

// Get category labels based on user role
export const getCategoryLabels = (userRole) => {
  if (userRole === "tenant") {
    return {
      communication: "Payment Reliability",
      maintenance: "Property Care",
      location: "Neighborhood Respect",
      value: "Lease Compliance",
      cleanliness: "Property Condition",
    };
  }

  // Default landlord labels
  return {
...
export const calculateCategoryAverages = (reviews) => {
  ...
  ratingCategories.forEach((category) => {
    const total = reviews.reduce(
      (sum, review) => sum + (review.categories[category.key] || 0),
      0
    );
...
export const filterReviews = (reviews, filters) => {
  ...
  return reviews.filter((review) => {
    ...
    if (
      filters.dateFrom &&
      new Date(review.reviewDate) < new Date(filters.dateFrom)
    ) {
      return false;
    }
    ...
    if (
      filters.dateTo &&
      new Date(review.reviewDate) > new Date(filters.dateTo)
    ) {
      return false;
    }
...
export const convertRatingToGrade = (rating) => {
  ...
  if (rating >= 1.0)
    return { grade: "U", label: "Unsatisfactory", color: "red" };
  return { grade: "U", label: "Unsatisfactory", color: "red" };
};
...
export const getGradeDistribution = (reviews) => {
  if (!reviews || reviews.length === 0) return {};

  const distribution = { "A+": 0, A: 0, B: 0, C: 0, U: 0 };
  reviews.forEach((review) => {
    const { grade } = convertRatingToGrade(review.rating);
...
export const calculateCategoryGrades = (reviews) => {
  if (!reviews || reviews.length === 0) return {};
